...
namespace TinaToolBox
{
    ...
    public:
        ...
        // 从资源加载 TTB 文件（不加密）
        static std::unique_ptr<TTBFile> loadFromResource(const std::string& resourceName,
                                                         const std::string& resourceType);
        // 从资源加载加密的 TTB 文件
        static std::unique_ptr<TTBFile> loadEncryptedFromResource(const std::string& resourceName,
                                                                  const std::string& resourceType, const AESKey& key);

        // 从内存数据加载 TTB 文件（不加密）
        static std::unique_ptr<TTBFile> loadFromMemory(const char* data, size_t size);
    ...
    private:
        ...
        // 读取配置区域
        static std::map<std::string, std::string> readConfig(std::istream& file,
                                                             uint32_t offset,
                                                             uint32_t size,
                                                             const AESKey* key = nullptr,
                                                             const AESIV* iv = nullptr);
